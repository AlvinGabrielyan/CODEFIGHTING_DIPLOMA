...
// ════════════════════════════════════════════════════════════════
//  ResultSaver  —  Դիպլոմային նախագծի արդյունքների պահպանում
//
//  ԿԱՐԵՎՈՐ: Բոլոր պրոյեկտները օգտագործում են ՆՈՒՅՆ DB ֆայլը։
//  DB տեղը (cross-platform):
//    Windows : C:/Users/<user>/AppData/Local/DiplomaApp/results.db
//    Linux   : ~/.local/share/DiplomaApp/results.db
//    macOS   : ~/Library/Application Support/DiplomaApp/results.db
// ════════════════════════════════════════════════════════════════

struct ProjectResult {
    QString projectId;
    QString starsStr;   // "★★★☆☆" ձևաչափ
    int     starsInt;   // 1–5
    QString savedAt;    // "dd.MM.yyyy HH:mm"
};

class ResultSaver {
public:

    // ── DB ճանապարհ (cross-platform, բոլոր պրոյեկտների համար ՆՈՒՅՆԸ) ──
    static QString dbPath() {
        // QStandardPaths::AppLocalDataLocation կարդում է
        // QCoreApplication::setApplicationName()-ը, ուստի մենք
        // ֆիքսված subdirectory ենք օգտագործում։
    ...
    // ── Level lock ────────────────────────────────────────────
    // Level 1: bucket, library   (միշտ բաց)
    // Level 2: bee, space        (պահանջում է Level 1 երկուսն էլ ✓)
    // Level 3: maze, ship        (պահանջում է Level 2 երկուսն էլ ✓)
